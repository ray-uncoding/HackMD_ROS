...
經過第二章我們對frame之間的轉換關係有初步了解，那接下來的問題就是要如何正確地把機械結構描述成數學結構。

而在數學上描述兩軸之間的相對關係，至少需要兩項參數:

1. 軸之間的正交距離（Link Length, a）

    + 描述相鄰兩軸的最短距離
    + 通常是在連桿上測量的直線距離

2. 軸之間的相對夾角（Link Twist, 𝛼）

    + 描述兩個軸的相對旋轉關係
    + 測量的是兩軸的方向角，定義在互相垂直的平面上
    
而機器人學中，由於joint的形式多種多樣(例如伺服馬達和滑軌)其對應的形式很難只用兩項參數就推算出準確的轉移矩陣，所以除了訂定旋轉軸(Z)的相對關係參數(a、𝛼)，還必須加入正交軸(X)之間的相對關係(d、𝜃)

3. 沿正交軸的位移（d）

    + 用於描述滑軌式（prismatic）關節的移動行為

4. 旋轉軸的旋轉角度（𝜃）

    + 用於描述旋轉式（revolute）關節的角度變化
